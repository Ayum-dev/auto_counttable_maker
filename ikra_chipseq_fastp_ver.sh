...
if [[ "$RUNINDOCKER" -eq "1" ]]; then
  ...
  PICARD_IMAGE=biocontainers/picard-tools:v2.18.25dfsg-2-deb_cv1
  ...
  PICARD="$DRUN $PICARD_IMAGE $PICARD"
...
#    fi
  ...
  # PE
    ...
    if [[ ! -f "${dirname_fq}${SRR}_1_val_1.fq.gz" ]]; then
      $FASTP --in1 ${dirname_fq}${SRR}_1.fastq.gz --int2 ${dirname_fq}${SRR}_2.fastq.gz \
  ...
  do
    ...
    # PE
      ...
      if [[ ! -f "bowtie2_output_${SRR}/${SRR}.bw" ]]; then
        ...
        samtools index bowtie2_output_${SRR}/${SRR}.trim.bam
        ...
        picard MarkDuplicates \
          I=bowtie2_output_${SRR}/${SRR}.trim.proper_pairs.bam \
          O=bowtie2_output_${SRR}/${SRR}.trim.proper_pairs.rmdup.bam \
          M=bowtie2_output_${SRR}/${SRR}.trim.proper_pairs.rmdup.bam.log.txt \
        ...
        -o bowtie2_output_${SRR}/${SRR}.bw --numberOfProcessors=max
        rm bowtie2_output_${SRR}/${SRR}.trim.sam
        #Picardによるフラグメント長（インサートサイズ）の分布の可視化
        mkdir picard_output_${SRR}
        picard CollectInsertSizeMetrics \
        INPUT=bowtie2_output_${SRR}/${SRR}.trim.proper_pairs.rmdup.bam \
        OUTPUT=picard_output_${SRR}/insert_size_metrics.txt \
        HISTOGRAM_FILE=picard_output_${SRR}/hist.pdf \
        MINIMUM_PCT=0
      fi
    fi
